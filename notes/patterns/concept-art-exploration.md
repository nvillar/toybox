...
6. **Contact sheet:**
   ...
   For browsing full-size images and their prompts, build a static HTML gallery from the experiment folder:
   ```bash
   python3 scripts/gen/gallery.py out/concept/example --out-dir out/gallery/example --title "Concept studies"
   ```
   Open `out/gallery/example/index.html` directly in a browser; no server is needed. Prefer presenting this local file rather than a localhost URL. The output directory must be new or empty and separate from the source. The gallery copies images and embeds sidecar metadata; it does not modify the archive. Keep the HTML and its relative media files together in ignored local output. Missing sidecars are flagged for rights review, not assumed permissive. If serving is genuinely required, bind only to loopback and serve only the generated gallery directory, never the repository root.

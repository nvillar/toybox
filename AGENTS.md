...
- **Keep artefacts private.** Generate into `out/` and keep third-party checkouts in `sandbox/tools/` (both git-ignored). Archive selected experiments, full briefs, prompts and provenance in the separate private `toybox-assets` library; production games get their own private repos. Public logs describe generic methods, not project identities or media. Local scratch is not a backup ([asset storage](notes/tools/asset-storage.md)).
- **Static review pages first.** For experiment galleries and similar presentations, prefer HTML that opens directly from disk without a running server (user preference, 2026-09-24). Keep media alongside it with relative links and embed metadata needed for browsing. Present the local HTML file, not a localhost URL. Use a server only when genuinely required or explicitly requested.
- **No secrets.** Reference API keys by environment variable *name* only.
- **Checkpoint completed work.** The user has authorized committing and pushing after each substantial update or completed work cycle (2026-09-23). Commit relevant knowledge and helpers here, and project-specific work in the private asset repo, after appropriate checks. Preserve unrelated changes; never force-push, rewrite history, publish private material or change repository visibility under this permission. Report blocked commits or pushes explicitly. A later instruction to pause or leave work uncommitted takes precedence.
